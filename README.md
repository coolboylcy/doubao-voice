...
- 按住右 Option 说话，松手上屏（PTT；可在设置里切换成左 Option）
- 短按所选 Option 进入持续录音，再按一下结束
- 录音中按 Esc 取消，什么都不会上屏
...
授权后菜单栏会出现「狗子 + 麦克风」模板图标，按住右 Option 即可开始。App 启动后需要约 10 秒预热
识别引擎，期间按键无效——这段时间只在刚开机时才会遇到。

菜单栏面板只保留运行状态、开始听写、打开主界面、设置、使用指南、常见问题、
反馈建议和退出。设置窗口分为通用、快捷键、语音识别、外观、高级、关于六部分；
热键、自动输入、启动提示、系统通知、浮层和菜单栏图标样式都会保存在本机。
...
录音时屏幕底部出现蓝白浮层：左侧是 12fps 的多姿态狗子动画，中间是实时波形，
右侧显示已用时 / 2 分钟上限，底部进度条与音频时间轴同步。狗子有 idle、listening、
speaking、finishing、delivered 状态，speaking 的动作幅度直接由实时麦克风音量驱动。
最后 10 秒进度与波形转为橙色警告态，单次录音上限 2 分钟。

松手后浮层进入 finishing，识别完成后以不到一秒的 delivered 勾选反馈收尾；整个
浮层是 non-activating panel，不会把焦点从 Claude Code、Cursor、终端或编辑器抢走。
...
有新版本时狗子会自己找上门：点「好，换上」就下载、校验、替换、重开，几秒钟
的事。不想现在弄可以「以后再说」，不想要这一版可以「跳过这版」——跳过的是
具体某一版，下一版出来照样提醒。

也可以自己去 设置 → 关于 → 更新 → 现在检查。

下载回来的包会先过一遍 `spctl -a -t install`，也就是你手动双击 DMG 时
Gatekeeper 做的那套检查：签名完整、证书链有效、确实被 Apple 公证过。不通过
就直接丢掉不装。这是整条链路上的安全闸门——中间人换了包，这里当场拦下。

换包本身交给一个临时脚本做：它先等旧进程退干净，再原子替换 `/Applications`
里的 App，然后重新打开。不在进程内直接覆盖自己，那样 copy 到一半时可执行
文件、资源、内嵌 helper 处于半新半旧状态，随便访问一下就崩。
